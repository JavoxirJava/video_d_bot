...
export async function registerMusicHandlers(bot) {

    // 1) /music komandasi
    bot.command('music', async (ctx) => {
        await ctx.reply('Qo‘shiq nomi yoki ijrochi ismini yuboring. Masalan: "Eminem Lose Yourself"');
        ctx.session ??= {};
        ctx.session.musicMode = true;
    });

    // 2) matn kelganda qidirish
    bot.on('text', async (ctx, next) => {
        ctx.session ??= {};
        if (!ctx.session.musicMode) return next();
    ...
    // 3) “MP3” tugmasi bosilganda
    bot.on('callback_query', async (ctx, next) => {
        const data = ctx.callbackQuery?.data || '';
        if (!data.startsWith('mget|')) return next();
